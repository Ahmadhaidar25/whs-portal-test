        ...
        return \DB::transaction(function () use ($request) {
           ...
           if ($request->hasFile('file')) {
                ...
                $imports = (new ExcelImport)->toArray($file);
                 //dd($imports);
                // Import
                $import = collect($imports[0]);

                // Header
                $header = $import->splice(0, 1)->first();
              
                // Datas
                $datas = $import->map(function($data) use($header) {
                ...
                if (!empty($datas) && $datas->count() > 0) {
                    if (!empty($datas)) {
                        ...
                        //dd($data1);
                        $data_arr[0] = array($data1);
    
                        if ($data1 == 0) {
                           foreach ($data as $key => $value1) {
                            $order =  Order::create([
                                'order_number' => $value1->order_number,
                                'purchase_order_number' => $value1->purchase_order_number,
                                'order_date' => $value1->order_time,
                                'order_time' => $value1->order_date,
                                'delivery_cycle' => $value1->delivery_cycle,
                            ...
                            foreach ($value1 as $key => $value2) {
                                $order_list =  OrderList::create([
                                    'order_id' => $order->id,
                                    'part_number' => $value2->part_number,
                                    'part_name' => $value2->part_name,
                                    'order_qty' => $value2->order_qty,
                                    'total_kanban' => $value2->total_kanban,
                                    'kanban_qty' => $value2->kanban_qty,
                                    'lp' => $value2->lp,
                                ]);
                                ...
                                for ($i = 0; $i < $order_list->total_kanban; $i++) {
                                    $d = Label::create([
                                        ...
                                        'kanban_number' => $order->order_number. ' ' .$value2->part_number. ' ' . ($i),
